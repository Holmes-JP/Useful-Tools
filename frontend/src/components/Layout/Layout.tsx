...
export default function Layout({ children }: { children: React.ReactNode }) {
    ...
    const navItems = [
        ...
        { path: '/audio', label: 'Audio Lab', icon: Music }, // 追加
    ...
    return (
        <div className="min-h-screen flex font-sans">
            <button 
                ...
                className="lg:hidden fixed top-4 left-4 z-50 p-2 bg-gray-900 text-white rounded-md shadow-md border border-gray-700"
            >
                {isSidebarOpen ? <X size={24} /> : <Menu size={24} />}
            </button>

            <aside className={clsx(
                "fixed inset-y-0 left-0 z-40 w-64 bg-black/50 backdrop-blur-xl border-r border-white/10 transition-transform duration-300 lg:translate-x-0 lg:static flex flex-col",
            ...
            <main className="flex-1 p-4 lg:p-8 overflow-y-auto h-screen relative">
                ...
                <div className="max-w-5xl mx-auto pt-10 lg:pt-0 pb-20">
                    {children}
                </div>
            </main>
