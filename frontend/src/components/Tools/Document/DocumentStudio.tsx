...
import { Archive, FileText, RefreshCw, X, File as FileIcon, Trash2 } from 'lucide-react';
        ...
        <div className="max-w-6xl mx-auto space-y-8">
            ...
            {(logs.length > 0 || results.length > 0) && (
                <div className="bg-black border border-gray-800 rounded-xl p-4 font-mono text-xs">
                    ...
                    {results.length > 0 && (<div className="border-t border-gray-800 pt-4"><div className="flex justify-between items-center mb-4"><span className="text-white font-bold text-lg">Result: {results.length} Files</span><button onClick={downloadAllZip} className="bg-white text-black px-6 py-2 rounded-full font-bold hover:bg-gray-200 flex items-center gap-2 shadow"><Archive size={16} /> Download All as Zip</button></div></div>)}
                </div>
            )}
        </div>
    );
