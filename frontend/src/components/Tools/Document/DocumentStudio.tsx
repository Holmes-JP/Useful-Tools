...
export default function DocumentStudio() {
    ...
    const results = pdf.pdfOutputUrl ? [{ url: pdf.pdfOutputUrl, name: 'merged.pdf' }] : [];
    const logs = pdf.pdfLog ? [pdf.pdfLog] : [];
        ...
        <div className="max-w-6xl mx-auto space-y-8">
            ...
            {(logs.length > 0 || results.length > 0) && (
                <div className="bg-black border border-gray-800 rounded-xl p-4 font-mono text-xs">
                    <div className="h-64 overflow-y-auto text-green-400 space-y-1 mb-4 custom-scrollbar p-2 bg-gray-900/50 rounded">{logs.map((l: string, i: number) => <div key={i}>{l}</div>)}</div>
