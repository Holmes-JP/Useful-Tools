            ...
            {!file ? (
                ...
                <div className="bg-surface border border-gray-700 p-6 rounded-xl space-y-6">
                    <video 
                        ...
                        className="w-full max-h-64 bg-black rounded border border-gray-700"
                        controls
                    />
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div>
                            ...
                            <input type="number" value={start} onChange={e => setStart(Number(e.target.value))} className="w-full bg-gray-900 text-white p-2 rounded border border-gray-600" />
                        </div>
                        <div>
                            <label className="text-xs text-gray-500 block mb-1">Duration (s)</label>
                            <input type="number" value={duration} onChange={e => setDuration(Number(e.target.value))} className="w-full bg-gray-900 text-white p-2 rounded border border-gray-600" />
                        </div>
                        <div>
                            <label className="text-xs text-gray-500 block mb-1">Width (px)</label>
                            <input type="number" value={width} onChange={e => setWidth(Number(e.target.value))} className="w-full bg-gray-900 text-white p-2 rounded border border-gray-600" />
                        </div>
                        <div>
                            <label className="text-xs text-gray-500 block mb-1">FPS</label>
                            <input type="number" value={fps} onChange={e => setFps(Number(e.target.value))} className="w-full bg-gray-900 text-white p-2 rounded border border-gray-600" />
                        </div>
                    </div>

                    <button 
                        onClick={() => createGif(file, start, duration, fps, width)}
                        disabled={isLoading}
                    ...
                    {isLoading && <div className="text-xs text-green-400 font-mono truncate">{log}</div>}

                    {gifUrl && (
                        <div className="text-center bg-gray-900 p-4 rounded-xl">
                            <img src={gifUrl} alt="GIF" className="mx-auto mb-4 rounded max-h-64" />
                            <a href={gifUrl} download="output.gif" className="inline-flex items-center gap-2 bg-white text-black px-6 py-2 rounded-full font-bold hover:bg-gray-200">
